...
5. A compiled-axiom audit rooted at `MathlibAnnex`, allowing exactly `propext`, `Classical.choice` and `Quot.sound`.
6. Clean tracked files, index and working tree after qualification.

The pinned environment is `leanprover/lean4:v4.32.0-rc1`, Mathlib `360da6fa66c1273b76b6b2d8c5666fd5ac2e3b56`, and the committed `lake-manifest.json`. The workflow and helper preserve separate results for identity, build, root import, Project import, compiled-axiom audit and cleanliness. Missing, skipped, cancelled or failed required checks do not constitute a successful qualification.
...
The repaired local `0.2.0-rc.1` candidate binds the exact 48-module delta, fresh entry parser result, direct root and content-addressed 48-module closure. The 13 authority-listed Lean blobs are LF-normalized, so the prior BuildResult is retained only as pre-repair evidence. The repaired source is qualified by an actual `lake build MathlibAnnex` and actual root, Mankiewicz and Sphere Rigidity import commands. The later owner-authorized private candidate must still run the independent compiled-axiom workflow before any public `v0.2.0` act.
